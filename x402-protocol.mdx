...
Not every tiered endpoint sells every tier. `POST /v1/entity/deep-check` sells two of
the three: `fresh` ($0.75, 750,000: PEP/RCA + Dutch insolvency register) and
`enhanced` ($1.50, 1,500,000, adds adverse media). It has no cache-only branch, so
`X-Response-Quality: cached` is quoted, charged and served as `fresh`.
