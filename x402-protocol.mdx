...
`enhanced` ($1.50, 1,500,000, adds adverse media) — it is unaffected by the #405
retirement above, since its `enhanced` tier genuinely reaches a source `fresh` does
not. It has no cache-only branch, so `X-Response-Quality: cached` is quoted, charged
and served as `fresh`.
...
[Configuration](./configuration.md)) are the exception: a CDP outage fails closed with
