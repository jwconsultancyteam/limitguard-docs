...
LimitGuard enforces rate limits to ensure fair access and API stability. They are abuse limits, not a quota you buy: every call is paid for, either per call with x402 or from an API key's prepaid balance. A key's tier (bought as a prepaid top-up) only selects these limits.

## Rate Limits by Tier

| Tier | Requests / Minute | Requests / 24 h | Monthly ceiling | Auth Method |
|------|:-----------------:|:---------------:|:---------------:|-------------|
| **Unauthenticated** | 10 / min per IP | No daily cap | None | No key, no payment |
| **Unpaid 402 quote** | 120 / min per IP | No daily cap | None | A request x402 answers with the price only |
| **x402 payer** | 60 / min per payer | No daily cap | None | `X-PAYMENT` header (pay per call) |
| **Free** | 10 / min | 500 | 500 | `lg_live_` API key, free tier (pays per call) |
| **Indie** | 30 / min | 1,000 | 1,000 | `lg_live_` API key, $29 prepaid top-up |
| **Starter** | 60 / min | 10,000 | 10,000 | `lg_live_` API key, $99 prepaid top-up |
| **Growth** | 120 / min | 50,000 | 50,000 | `lg_live_` API key, $299 prepaid top-up |
| **Pro** | 300 / min | 250,000 | 250,000 | `lg_live_` API key, $999 prepaid top-up |

<Note>
The per-minute and 24-hour windows are rolling. The monthly ceiling is an abuse cap on an API key, not your prepaid balance: it counts the key's calls in the calendar month and resets on the first day of each month (UTC). A call paid per call with x402 is never refused by it, even when it carries a key. Entity checks also have a per-entity limit per caller, against enumeration.
</Note>
...
### Example: Monthly Ceiling Reached

```json
{
  "detail": "Monthly request ceiling reached (500/500). This is an abuse cap, not your prepaid balance; it resets next month."
}
```

<Warning>
Always read the `Retry-After` header rather than hard-coding a wait time: it is 60 seconds for a per-minute limit and 3600 seconds for the rolling 24-hour limit. The monthly ceiling lifts at the start of the next calendar month (UTC); until then, pay per call with x402, which it never refuses.
</Warning>
...
| Monthly ceiling (abuse cap) | Yes (by tier) | No |
...
<CardGroup cols={2}>
  <Card title="Upgrade Your Plan" icon="arrow-up" href="/authentication">
    Top up to Indie, Starter, Growth, or Pro for higher per-minute, daily and monthly limits.
  </Card>
  <Card title="Switch to x402" icon="credit-card" href="/x402-protocol">
    Pay per call with USDC. No daily or monthly cap; 60 requests per minute per payer.
  </Card>
</CardGroup>
