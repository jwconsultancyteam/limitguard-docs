...
| `429` | Too Many Requests | Per-minute, rolling 24-hour or monthly-ceiling limit exceeded |
...
LimitGuard enforces three limits per API key: a per-minute limit, a rolling 24-hour limit and a monthly request ceiling (an abuse cap that resets on the first of each month UTC). A verified x402 payer has a per-minute limit only.
...
**Fix:** Wait for `Retry-After`, top up to a higher tier for a higher limit, or pay per call with x402, which has no daily cap.

### Monthly Ceiling Reached

A key that reaches its monthly request ceiling gets:
...
**Fix:** Wait for the first of the next month (UTC), top up to a higher tier, or pay per call with x402: a paid call is never refused by the ceiling.
