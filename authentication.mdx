...
A paid-tier API key skips the per-call x402 payment: each call is debited at list price from the key's prepaid balance instead. A free key still pays per call. The tier only selects the key's rate limits and monthly request ceiling, an abuse cap that resets on the first of each month UTC (see [Rate Limits](/guides/rate-limits)).
...
| Tier | Top-up (credited to the prepaid balance) | Rate limits | Monthly ceiling |
|------|------------------------------------------|-------------|-----------------|
| `free` / `sandbox` | none: calls are paid per call over x402 | 10 / min, 500 / 24 h | 500 |
| `indie` | $29 | 30 / min, 1,000 / 24 h | 1,000 |
| `starter` | $99 | 60 / min, 10,000 / 24 h | 10,000 |
| `growth` | $299 | 120 / min, 50,000 / 24 h | 50,000 |
| `pro` | $999 | 300 / min, 250,000 / 24 h | 250,000 |
